...
/**
 * [classic 테마] PC 헤더
 * 상위 includes/header.php 에서 include — $site, $current_user, $cats_l1 등 사용 가능.
 */
?>
<!-- 상단 유틸리티 + 메인 헤더 (함께 고정) -->
...
<header class="bg-white/95 backdrop-blur border-b border-gray-200">
  <div class="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between gap-6">
    ...
    <button class="md:hidden p-2 -ml-2 text-primary" @click="mobileMenu = true" aria-label="메뉴">
    ...
    <a href="/" class="flex items-center gap-2 shrink-0">
      ...
      <div class="leading-tight">
        <div class="font-extrabold text-base md:text-lg text-primary tracking-tight"><?= h($site['name']) ?></div>
        <div class="hidden md:block text-[11px] text-gray-500"><?= h($site['tagline']) ?></div>
      </div>
    </a>

    <form action="/shop/list.php" method="get" class="hidden md:flex flex-1 max-w-xl mx-4 relative">
      <input type="text" name="q" placeholder="찾으시는 배터리를 검색해보세요" value="<?= h($_GET['q'] ?? '') ?>"
             class="w-full h-11 pl-4 pr-12 rounded-full border-2 border-primary focus:outline-none focus:ring-2 focus:ring-accent text-sm">
      <button type="submit" class="absolute right-1 top-1 bottom-1 px-4 rounded-full bg-primary text-white hover:bg-primary-light transition">
        검색
      </button>
    </form>

    <div class="flex items-center gap-2 md:gap-4 shrink-0">
      <a href="/cart.php" class="relative p-2 text-primary hover:text-accent-dark transition" aria-label="장바구니">
        ...
        <?php if (cart_count() > 0): ?>
        <span class="absolute -top-0.5 -right-0.5 min-w-[20px] h-5 px-1 rounded-full bg-accent text-primary text-[11px] font-bold flex items-center justify-center">
          <?= cart_count() ?>
        </span>
        <?php endif; ?>
      </a>
  ...
  <nav class="hidden md:block bg-white border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-4">
      <ul class="flex items-center gap-1 h-12 text-sm font-semibold">
        <li><a href="/" class="px-3 py-2 hover:text-accent-dark text-primary">HOME</a></li>
        <?php foreach ($cats_l1 as $c): ?>
        <li><a href="<?= h(url('/shop/list.php', ['ca_id' => $c['ca_id']])) ?>"
               class="px-3 py-2 hover:text-accent-dark text-gray-700 hover:text-primary"><?= h($c['ca_name']) ?></a></li>
        <?php endforeach; ?>
        <li class="ml-2 pl-3 border-l">
          <a href="/cases/list.php" class="px-3 py-2 hover:text-accent-dark text-gray-700 hover:text-primary">납품사례</a>
        </li>
        <li><a href="/blog/list.php" class="px-3 py-2 hover:text-accent-dark text-gray-700 hover:text-primary">블로그</a></li>
        <li><a href="/faq.php" class="px-3 py-2 hover:text-accent-dark text-gray-700 hover:text-primary">FAQ</a></li>
      </ul>
    </div>
  </nav>
</header>
...
<div x-show="mobileMenu" x-cloak class="fixed inset-0 z-50 md:hidden" x-transition.opacity>
  ...
  <aside class="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-xl flex flex-col"
    ...
    <form action="/shop/list.php" method="get" class="p-4 border-b">
      <input type="text" name="q" placeholder="검색"
             class="w-full h-10 px-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-accent text-sm">
    </form>
    <nav class="flex-1 overflow-y-auto py-2">
      ...
      <?php foreach ($cats_l1 as $c): ?>
        <a href="<?= h(url('/shop/list.php', ['ca_id' => $c['ca_id']])) ?>"
           class="block px-4 py-3 font-semibold text-gray-800 hover:bg-gray-50 border-t"><?= h($c['ca_name']) ?></a>
        <?php foreach (category_tree()[$c['ca_id']] ?? [] as $sub): ?>
          <a href="<?= h(url('/shop/list.php', ['ca_id' => $sub['ca_id']])) ?>"
             class="block pl-8 pr-4 py-2 text-sm text-gray-600 hover:bg-gray-50">└ <?= h($sub['ca_name']) ?></a>
        <?php endforeach; ?>
      <?php endforeach; ?>
