...
/**
 * [classic 테마] 홈페이지 본문
 * 사용 변수: $banners, $top_categories, $best_products, $new_products, $recent_posts
 */
?>
<!-- ───────── HERO 슬라이더 ───────── -->
...
<section class="relative bg-primary-dark text-white overflow-hidden">
  <div class="swiper hero-swiper">
        ...
        <div class="aspect-[16/9] md:aspect-[16/6] min-h-[360px] md:min-h-[480px] relative">
          ...
          <div class="absolute inset-0 flex flex-col justify-center <?= $align_cls ?>">
            <div class="max-w-7xl w-full mx-auto px-4 md:px-8 flex flex-col <?= $align_cls ?>">
              ...
              <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/25 text-accent text-xs font-bold mb-4 backdrop-blur-sm">
              ...
              <?php if ($b['cta_text']): ?>
              <a href="<?= h($b['cta_url'] ?: '#') ?>"
                 class="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg bg-accent text-primary font-bold hover:bg-accent-light transition shadow-lg w-fit">
                <?= h($b['cta_text']) ?> →
              </a>
              <?php endif; ?>
            </div>
    ...
    <button class="hero-swiper-prev hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-white/20 hover:bg-white/40 backdrop-blur items-center justify-center text-white text-xl">‹</button>
    <button class="hero-swiper-next hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-white/20 hover:bg-white/40 backdrop-blur items-center justify-center text-white text-xl">›</button>
    <?php endif; ?>
  </div>
  <style>
...
<section class="border-b">
  <div class="max-w-7xl mx-auto px-4 py-12 md:py-14 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
    <?php foreach ([['🚚','전국 무료배송','5만원 이상 주문 시'],['⏱️','당일 발송','평일 15시 이전'],['🛡️','정품 보장','불량 시 100% 교환'],['📞','전문 상담','평일 09 - 18시']] as $bn): ?>
    <div class="flex flex-col items-center gap-2">
      <span class="text-5xl md:text-6xl"><?= $bn[0] ?></span>
      <div class="text-base md:text-lg font-bold"><?= $bn[1] ?></div>
      <div class="text-xs md:text-sm text-gray-500"><?= $bn[2] ?></div>
    </div>
    <?php endforeach; ?>
  </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 py-12 md:py-16">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <a href="<?= h(url('/shop/list.php', ['ca_id' => $c['ca_id']])) ?>"
       class="group relative aspect-square rounded-2xl overflow-hidden bg-gradient-to-br <?= $v['gradient'] ?> text-white p-5 flex flex-col justify-between hover:scale-[1.02] transition shadow-md">
...
<section class="bg-gray-50 py-12 md:py-16">
  <div class="max-w-7xl mx-auto px-4">
    <div class="flex items-end justify-between mb-8">
      ...
      <a href="<?= h(url('/shop/list.php')) ?>" class="hidden md:inline-flex items-center gap-1 text-sm text-primary font-semibold hover:text-accent-dark">전체보기 →</a>
    </div>
...
<section class="bg-gray-50 py-12 md:py-16">
  <div class="max-w-7xl mx-auto px-4">
    <div class="flex items-end justify-between mb-8">
      ...
      <a href="/blog/list.php" class="hidden md:inline-flex items-center gap-1 text-sm text-primary font-semibold hover:text-accent-dark">블로그 전체보기 →</a>
    </div>
    <div class="grid md:grid-cols-2 gap-5">
      <?php foreach ($recent_posts as $post):
        $img = $post['thumbnail'] ?: 'https://placehold.co/800x500/0A2540/FFC107?text=Blog';
      ?>
      <a href="<?= h(url('/blog/view.php', ['id' => $post['id']])) ?>" class="product-card group block bg-white rounded-2xl overflow-hidden border">
        <div class="grid md:grid-cols-[180px_1fr]">
          <div class="aspect-[16/10] md:aspect-auto bg-gray-100 overflow-hidden">
            <img src="<?= h($img) ?>" alt="" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
          </div>
          <div class="p-4 md:p-5">
            <div class="text-[11px] text-gray-400 mb-1"><?= h(date('Y.m.d', strtotime($post['created_at']))) ?></div>
            <h3 class="font-extrabold text-base text-primary line-clamp-2 leading-snug group-hover:text-accent-dark transition"><?= h($post['title']) ?></h3>
            <?php if ($post['summary']): ?>
...
<section class="max-w-7xl mx-auto px-4 mb-16">
  <div class="rounded-2xl bg-gradient-to-r from-primary to-primary-dark text-white p-8 md:p-12 grid md:grid-cols-3 items-center gap-6 relative overflow-hidden">
    ...
    <div class="md:col-span-2 relative">
      ...
      <p class="text-white/80 text-sm leading-relaxed">택시 사업자, 산업체, 솔라 시공업체 등 100개 이상 대량 주문 시 별도 견적 + 추가 할인 + 직배송 지원</p>
    </div>
    <div class="relative">
      <a href="#" class="inline-flex items-center justify-center w-full md:w-auto px-8 py-3 rounded-lg bg-accent text-primary font-bold hover:bg-accent-light transition">견적 문의하기</a>
    </div>
  </div>
</section>
